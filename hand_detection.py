...
def main():
    ...
    # cap_device = args.device  # Camera device index (unused when using GO2 camera)
    ...
    dispatcher = GestureDispatcher(build_gesture_actions(sport_client), cooldown=2.0)
    ...
    while True:
        ...
        if code != 0 or not data:
            ...
            if image is None:
                ...
                if (cap_width and cap_height) and (w != cap_width or h != cap_height):
                    image = cv.resize(
                        image, (cap_width, cap_height), interpolation=cv.INTER_AREA
                    )
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(
                ...
                # Dispatch gesture action asynchronously with cooldown
